...
@st.cache_data
def load_data():
    # Get the folder where this movie.py file is located
    base_dir = os.path.dirname(os.path.abspath(__file__))

    # Load movie data from the same folder as movie.py
    movies = pd.read_csv(
        os.path.join(base_dir, "movie_data.csv")
    )

    # Load TF-IDF matrix from the same folder as movie.py
    with open(
...
def top_rated(
    ...
    df = movies[
        movies["rating_count"] >= min_rating_count
    ].copy()
...
with st.sidebar:
    ...
    st.caption(
        "Filters apply to recommendations and "
        "to the Top Rated / Popular tabs."
    )
    ...
    if movie_name:
        ...
        if results.empty:
            ...
            for _, row in results.iterrows():
                ...
                cols[0].markdown(
                    f"**{row['title']}**  \n"
                    f"{row['genres'].replace('|', ', ')}"
                )

                cols[1].markdown(
                    f"Match: "
                    f"{row['similarity'] * 100:.0f}%"
                )
